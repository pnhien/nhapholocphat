...
function fnc_doAddVideoToSubscriptions(){
    ...
    <?php echo $this->Form->create('TCustomer', array('id'=>'TVideoIndexForm', 'url' => array ('controller' => 'manageReup', 'action' => 'doDeleteVideo'), 'role' => 'form', 'novalidate' => true)); ?>
    <div class="fl">
    <?php 
	$cnt = 0;
	if(isset($videos)){
		...
		if(isset($videos) && (is_array($videos) || is_object($videos))){
			...
			foreach ($videos as $video){
				...
				if($hdn_id_video_id_list == ""){
					$hdn_id_video_id_list = $video['TVideoReup']['VIDEO_ID'];
				}
				else{
					$hdn_id_video_id_list = $hdn_id_video_id_list . "," . $video['TVideoReup']['VIDEO_ID'];
				}
				if($cnt % 2 == 0){
	        		echo "<tr class=''>";
	        	}else{
		    ...
		    <?php 
				...
				$ga = '';
		    	if($video['TVideoReup']['GA'] == 1){
		    		$ga = "(GA)";	
		    	}
		    	echo "<input type='hidden' id='hdn_id_video_id$cnt' value='".$video['TVideoReup']['VIDEO_ID']."'>";
	  	    ?>
			<td style="text-align:center"><?php echo $cnt;?></td>
            <td><img class="img-thumbnail" src="<?php echo $video['TVideoReup']['THUMBNAILS'];?>" style="max-width:inherit;width:90px"></td>
			<td><a target="_blank" title="<?php echo $video['TVideoReup']['TITLE'];?>" href="https://www.youtube.com/watch?v=<?php echo $video['TVideoReup']['VIDEO_ID'];?>"><?php echo $video['TVideoReup']['TITLE']. "(".$video['TVideoReup']['DURATION'].")";?></a></td>
			<td><a target="_blank" title="<?php  echo $video['TVideoReup']['CHANNEL_TITLE']; ?>" href="https://www.youtube.com/channel/<?php echo $video['TVideoReup']['CHANNEL_ID'];?>"><?php  if(strlen($video['TVideoReup']['CHANNEL_TITLE']) == 0){ echo $video['TVideoReup']['CHANNEL_ID']; }else{ echo $video['TVideoReup']['CHANNEL_TITLE'];} ?></a></td>
            <td><?php echo isset($video['TVideoReup']['ATTRIBUTION']) ? $video['TVideoReup']['ATTRIBUTION'] . $ga : "Only Vip Account"; //echo getMetaAttribution("https://www.youtube.com/watch?v=".$searchResult['id']);?></td>
			<td><?php 
				if(strlen($video['TVideoReup']['REJECTION_REASON']) == 0){
					echo "Bình Thường";
				}
				else{
					echo $video['TVideoReup']['REJECTION_REASON'];
				}
			
			?></td>
			<td><?php 
			if($video['TVideoReup']['LICENSED_CONTENT'] == 1){
				echo "Có";
			}
			else{
				echo "Không";
			...
			<td><?php echo $video['TVideoReup']['REGION_RESTRICTION_BLOCKED'];?></td>
			<td>
				<span class="info"><span class="view">&nbsp;</span><?php echo getCountShot($video['TVideoReup']['VIEW_COUNT']);?></span>
				<span class="info"><span class="like">&nbsp;</span><?php echo getCountShot($video['TVideoReup']['LIKE_COUNT']);?></span>
				<span class="info"><span class="dislike">&nbsp;</span><?php echo getCountShot($video['TVideoReup']['DISLIKE_COUNT']);?></span>
				<span class="info"><span class="subcribe">&nbsp;</span><?php echo getCountShot($video['TVideoReup']['FAVORITE_COUNT']);?></span>
				<span class="info"><span class="comment">&nbsp;</span><?php echo getCountShot($video['TVideoReup']['COMMENT_COUNT']);?></span><br>
                <div class="clearfix"></div>
                Publis: <?php echo $video['TVideoReup']['PUBLISHED_AT'];?>
            </td>
            <td>
            	<?php echo $video['TVideoReup']['DIMENSION'] . "<br>". $video['TVideoReup']['DEFINITION'] . "<br> " . $video['TVideoReup']['DELETE_YMD'];?>
            </td>
